# -*- coding: utf-8 -*-
"""
Coletor de produtos "Matinais" do Atacadão via API VTEX.

O Atacadão (Grupo Carrefour Brasil) usa a mesma plataforma VTEX do Carrefour,
portanto o mesmo padrão de API se aplica (sem Playwright).

Categorias usadas (árvore: Padaria e matinais > Aveias e cereais):
  - Cereal matinal
  - Aveia
  - Granola
...
Gera/atualiza na raiz do projeto:
  ...
  - precos_atacadao_matinais.json     (preço/estoque)

Uso:
    python scrap_atacadao.py
"""
...
# ---------------------------------------------------------------------------
CATEGORIAS = [
    "padaria-e-matinais/aveias-e-cereais/cereal-matinal",
    "padaria-e-matinais/aveias-e-cereais/aveia",
    "padaria-e-matinais/aveias-e-cereais/granola",
]
PAGINA_TAMANHO = 50
INTERVALO_SLEEP = 1.0
TIMEOUT = 15
...
# Marcador nas URLs de imagem para identificar produtos desta loja no catálogo
MARCADOR_LOJA = "atacadaobr"

ARQUIVO_PRODUTOS = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "produtos_matinais.json"
)
ARQUIVO_PRECOS = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "precos_atacadao_matinais.json"
)
...
def buscar_pagina(categoria: str, offset: int, limit: int) -> list:
    ...
    url = f"https://www.atacadao.com.br/api/catalog_system/pub/products/search/{categoria}"
...
def main() -> None:
    ...
    print(f"=== Coletor Matinais Atacadão (VTEX) ===")
    print(f"Data da coleta: {hoje}")
    print(f"Categorias: {', '.join(CATEGORIAS)}")
    print("-" * 70)

    # Carrega catálogo existente e remove produtos antigos do Atacadão
    # (mantém produtos de outras lojas, ex. Carrefour e Pão de Açúcar)
    produtos_antes = carregar_existentes(ARQUIVO_PRODUTOS)
    produtos_outras_lojas = {
        ean: p for ean, p in produtos_antes.items()
        if MARCADOR_LOJA not in (p.get("imagem_url") or "")
    }
    removidos = len(produtos_antes) - len(produtos_outras_lojas)
    print(f"Catálogo existente: {len(produtos_antes)} | Atacadão removidos p/ recálculo: {removidos} | preservados (outras lojas): {len(produtos_outras_lojas)}")

    # Preços do Atacadão são recalculados do zero
    precos_exist: dict = {}
    ...
    for categoria in CATEGORIAS:
        ...
        while True:
            ...
            for prod in produtos:
                ...
                # A API devolve muitos itens inativos (preço 0). Só entram
                # produtos com preço real.
                ...
                # apenas registra o preço do Atacadão para este EAN
    ...
    print(f"  -> Sem EAN válido / inativos: {ignorados_sem_ean}")
    print(f"Preços Atacadão salvos: {len(precos_exist)} (novos: {novos_precos})")
    print(f"Arquivos:")
